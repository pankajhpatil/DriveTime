...
class EnrollmentComponent extends Component{
    ...
    getInstructors = async (data) => {
        console.log("Api called for instructors");
        ...
        if((status === 'Instructors are Available' || response.data.statusText === 'Instructors are Available') && response.data.result.data.length >= data.planNo){
